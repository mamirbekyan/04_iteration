...
#| message: false 
#| warning: false #there are two ways of doing the same thing
#if you have many things to edit,easier to do here than in the top line
...
```{r density plot} 
nematode %>%
  pivot_longer(cols = mel_s:re) %>% #pivot from mels to re, and name a name and value column
  ggplot(aes(x = value)) +
  geom_density() +
  facet_wrap(~name, scales = "free") #facet wrap separating by name. Scales allows the axis to be flexible, free means both x and y
...
```{r}
nematode %>%
  mutate(rndvi = ndvi(nir,r)) #this is how you assign the variables, with a comma. rndvi is the column name, and the contents of the content equals the output of that function, the inputs for which are listed afterwards
...
df <- data.frame(mean = mean,#first mean is the column name, second mean is the value from the object created previously
                 cv = cv
                 ) 
  # Return the data frame
return(df)  #so that the output is the dataframe
}  
...
```{r}
meancv(nematode_w$rndvi) #to get the data.frame but only the one column out of it
```
...
A `while` loop will operate under a condition.
...
```{r}
for (i in 3:ncol(nematode_w)) { #i is a convention, do the loop from column 3 to however many columns there are in nematode_w. We dont want to calculate mean and cv of the first two. Lets say position 10 (column 10 of nematode_w) was not numerical, we can recreate an object with only numerical columns - i.e. function in dplyr maybe "across" which you can make it only keep columns that are numerical.
...
varname <- colnames[i] #call this index
  # Calculating mean and cv
statsum <- meancv(nematode_w[[i]]) #double brackets turns the tibble/data.frame into a vector
...
nematode_mods <- nematode_w %>%
  ...
  nest()%>% #all of the observations that had eggs_r and b, for example, are within a single dataframe. There are dataframes within the dataframe. If you wanted to label the column by number, you can mutate and add a column with values 1:150 to know which row to call if youre interested in a particular tibble.
  mutate(lm = map(data, #this is the column "data" in the main dataset that you want to iterate over (the tibble). lm will be the name of the column
        ~lm(resp_val ~ exp_val, # we want to make a linear model for the resp nand exp values from the data within the tibble
            data = .x #the lm function is expecting a data function (here, data is an argument), and .x is the convention that the map function recognizes as the original "data" column name. There's nothing else you could have written here, its just a convention of the function to rewrite it. It IS redundant.
            )
  ))  %>%     
  # Applying linear model to each element
  mutate(r2 = map(lm, #the iterating column is lm. if we write map_dbl we get the actual value. Otherwise we get a nested number, which we can also just pull out using summary(nematode_mods$lm[[1]])
                  ~summary(.x)$r.squared
                  ))
...
nematode_mods_sel <- nematode_mods %>%
  unnest(r2) %>%
  group_by(resp_var) %>% #want the best model for the response variable, and pull out only the best explanatory variable model by the top R2 for each response variable related to nematode variables. 
  slice_max(r2)
...
nematode_mods_sel_plots <- nematode_mods_sel %>%
  ...
  mutate(splot = map2(data, r2,
                      ~ggplot(data = .x, #the first one, data
                              aes(x = exp_val,
                                  y = resp_val)
                              ) +
                        geom_point() +
